...
window.onload = function () {
  ...
  let speed = 5; // speed variable

  function Point() {
    this.ang = 2 * PI * random();
    this.dang = (-0.5 + random()) / speed; // Speed control here
    this.r = (2 * L) / 6; // Change here radius
  ...
  function animate() {
    ...
    for (let p = 0; p < n; p++) {
      ...
      ctrls[p].update();
      ctx.strokeStyle = `hsl(${round(180 + p)}deg, ${p + 50}%, 70%)`;
    }
    ctx.shadowBlur = (L * 30) / 432;
    ctx.shadowColor = `hsl(${round(180 + c)}deg, ${c}%, 50%)`;
